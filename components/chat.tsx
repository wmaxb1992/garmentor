...
type GeneratePatternToolOutput =
  | {
      ok: true;
      id: string;
      gcdUrl: string;
...
function PatternToolPart({
  state,
  output,
}: {
  state: string;
  output?: GeneratePatternToolOutput;
}) {
  const { addModel, attachPattern, setActive, state: ws } = useWorkspace();
  ...
  useEffect(() => {
    ...
    addModel({
      ...
      description: output.description,
      sourceImageUrl: output.sourceImageUrl,
      gcdUrl: output.gcdUrl,
    });
    fetch(output.gcdUrl)
      .then((r) => (r.ok ? r.json() : null))
      .then((gcd) => {
        if (cancelled || !gcd) return;
        attachPattern(output.id, output.gcdUrl, gcd);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
    };
  }, [state, output, addModel, attachPattern]);

  if (state === "input-streaming" || state === "input-available") {
    return (
      <div className="rounded-xl border border-dashed border-zinc-300 bg-white/50 px-3 py-2 text-xs text-zinc-500 dark:border-zinc-700 dark:bg-zinc-900/40">
        Generating sewing pattern… this can take 30–90s on a cold GPU.
      </div>
    );
  }
  if (state === "output-available" && output) {
    ...
    const isActive = ws.activeModelId === output.id;
    return (
      <div className="rounded-xl border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-950">
        <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
          {output.panelCount} panel{output.panelCount === 1 ? "" : "s"} generated
        </div>
        <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
          Open the Pattern tab to view + export DXF.
        </div>
