import torch
import torch.nn as nn
import torch.nn.functional as F
from model.pointnet2_ssg_wo_normals.pointnet2_utils import PointNetSetAbstraction


class get_model(nn.Module):
    def __init__(self,num_class,normal_channel=True):
        super(get_model, self).__init__()
        in_channel = 6 if normal_channel else 3
        self.normal_channel = normal_channel
        self.sa1 = PointNetSetAbstraction(npoint=512, radius=0.2, nsample=32, in_channel=in_channel, mlp=[64, 64, 128], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=128, radius=0.4, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256], group_all=False)
        self.sa3 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=256 + 3, mlp=[256, 512, 1024], group_all=True)
        self.fc1 = nn.Linear(1024, 512)
        self.bn1 = nn.BatchNorm1d(512)
        self.drop1 = nn.Dropout(0)
        self.fc2 = nn.Linear(512, 256)
        self.bn2 = nn.BatchNorm1d(256)
        self.drop2 = nn.Dropout(0)
        self.fc3 = nn.Linear(256, num_class)

    def forward(self, xyz):
        B, _, _ = xyz.shape
        if self.normal_channel:
            norm = xyz[:, 3:, :]
            xyz = xyz[:, :3, :]
        else:
            norm = None
        l1_xyz, l1_points = self.sa1(xyz, norm)
        l2_xyz, l2_points = self.sa2(l1_xyz, l1_points)
        l3_xyz, l3_points = self.sa3(l2_xyz, l2_points)
        x = l3_points.view(B, 1024)
        x1 = self.fc1(x)
        x1 = self.bn1(x1)
        x1 = F.relu(x1)
        x1 = self.drop1(x1)

        x2 = self.fc2(x1)
        logits = x2  # fc2的输出作为logits返回
        x2 = self.bn2(x2)
        x2 = F.relu(x2)
        x2 = self.drop2(x2)


        x3 = self.fc3(x2)
        x_out = F.log_softmax(x3, -1)

        return x, logits, l3_points



class get_loss(nn.Module):
    def __init__(self):
        super(get_loss, self).__init__()

    def forward(self, pred, target, trans_feat):
        total_loss = F.nll_loss(pred, target)

        return total_loss



# 测试代码
if __name__ == "__main__":
    # 假设有40类，输入点云为B=2, C=3, N=1024
    num_class = 40
    B, C, N = 2, 3, 1024
    model = get_model(num_class=num_class, normal_channel=False)
    # 加载预训练模型权重
    checkpoint_path = "/data/qj/Pointnet_Pointnet2_pytorch/log/classification/pointnet2_ssg_wo_normals/checkpoints/best_model.pth"
    checkpoint = torch.load(checkpoint_path, map_location='cuda',weights_only=False)
    if 'model_state_dict' in checkpoint:
        model.load_state_dict(checkpoint['model_state_dict'])
    else:
        model.load_state_dict(checkpoint)
    model.eval()

    # 构造输入并推理
    xyz = torch.ones(B, C, N)
    print("xyz shape:", xyz.shape)
    with torch.no_grad():
        logits, out, l3_points = model(xyz)
    torch.set_printoptions(threshold=float('inf'), linewidth=200, sci_mode=False)
    print("logits shape:", logits.shape)
    print("输出log_softmax shape:", out.shape)
    print("l3_points shape:", l3_points.shape)
    print("logits:", logits)
    # 假设标签
    target = torch.zeros(B, dtype=torch.long)
    criterion = get_loss()
    loss = criterion(out, target, None)
    print("loss:", loss.item())