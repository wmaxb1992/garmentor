...
class ResNetEncoder(nn.Module):
    """基于 ResNet 的多段线编码器，支持panel级别点和edge级别点的融合，panel处理更深，edge处理更浅"""

    def __init__(self, input_dim, latent_dim, num_points, panel_N, hidden_dim=256, num_layers=4):
        super(ResNetEncoder, self).__init__()
        self.num_points = num_points  # 边级别采样点数
        self.panel_N = panel_N        # panel级别采样点数

        # Panel点的特征投影和处理（更多层）
        self.panel_input_fc = nn.Linear(input_dim, hidden_dim)
        self.panel_blocks = nn.ModuleList(
            [
                nn.Sequential(
                ...
                for _ in range(1)  # panel处理层数更多
            ]
        )
        # 边级别点的特征投影和处理（更少层）
        self.edge_input_fc = nn.Linear(input_dim, hidden_dim)
        ...
        # 输出映射到潜在空间
        self.output_fc = nn.Linear((num_points + panel_N) * hidden_dim, latent_dim)

    def forward(self, x):
        """
        x: (bs, num_points + panel_N, 2)
        前num_points为edge级别点，后panel_N为panel级别点
        """
        bs = x.shape[0]
        edge_points = x[:, :self.num_points, :]   # (bs, num_points, 2)
        panel_points = x[:, self.num_points:self.num_points+self.panel_N, :]  # (bs, panel_N, 2)

        # Panel点处理（更深）
        panel_feat = self.panel_input_fc(panel_points)  # (bs, panel_N, hidden_dim)
        for block in self.panel_blocks:
            residual = panel_feat
            panel_feat = block(panel_feat) + residual
            panel_feat = F.gelu(panel_feat)

        # 边点处理（更浅）
        edge_feat = self.edge_input_fc(edge_points)  # (bs, num_points, hidden_dim)
        for block in self.edge_blocks:
            residual = edge_feat
            edge_feat = block(edge_feat) + residual
            edge_feat = F.gelu(edge_feat)

        # 融合
        feat = torch.cat([edge_feat, panel_feat], dim=1)  # (bs, num_points+panel_N, hidden_dim)
        for block in self.fusion_blocks:
            residual = feat
            feat = block(feat) + residual
            feat = F.gelu(feat)

        # 展平并映射到潜在空间
        feat = feat.reshape(bs, -1)  # (bs, (num_points+panel_N)*hidden_dim)
        return self.output_fc(feat)  # (bs, latent_dim)
...
class VQAutoCodec(nn.Module):
    def __init__(self, input_dim, latent_dim, hidden_dim, N, panel_N, num_layers, n_heads, codebook_size, num_quantizers, decay, is_fsq, fsq_levels):
        super(VQAutoCodec, self).__init__()
        self.encoder = ResNetEncoder(input_dim, latent_dim, N, panel_N, hidden_dim, num_layers)
...
if __name__ == "__main__":
    ...
    # 构造输入: x为(bs, N+panel_N, 2)
    batch = torch.randn(32, 32+128, 2)  # (bs=32, N+panel_N=160, 2)
