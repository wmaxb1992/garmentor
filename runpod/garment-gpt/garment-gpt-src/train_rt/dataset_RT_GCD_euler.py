    ...
    def __init__(self, pattern_jsons_dirs: Union[str, List[str]], gcd_split_json: Union[str, List[str]], gcd_root_dir: Union[str, List[str]], split_type: Union[str, List[str]]):
        ...
        if gcd_split_json:
            for idx, split_f in enumerate(gcd_split_json):
                ...
                split_type_set = set(split_type)
                if "train" in split_type_set and "training" not in split_type_set:
                    split_type_set.add("training")
                if "training" in split_type_set and "train" not in split_type_set:
                    split_type_set.add("train")
                if "val" in split_type_set and "validation" not in split_type_set:
                    split_type_set.add("validation")
                if "validation" in split_type_set and "val" not in split_type_set:
                    split_type_set.add("val")
                split_type = list(split_type_set)
                ...
                for st in split_type:
                    ...
                    for path in split_paths:
                        ...
                        if "editing" in cur_gcd_root:
                            full_path = os.path.join(cur_gcd_root, path, "editing_pattern_dict_specification.json")
                        else:
        ...
        for file_path in tqdm(json_files):
            ...
            if ("GarmentCodeData" in file_path or "GCD" in file_path or 'editing' in file_path) and "specification.json" in file_path:
    ...
    def _process_specification(self, file_path: str):
        with open(file_path, "r") as f:
            ...
            for panel_name, panel_data in panels.items():
                ...
                self.rotations.append(torch.tensor(rotation_euler, dtype=torch.float32))
...
if __name__ == "__main__":
    split_file =  "/data/qj/codec_codebook/extract_indices/aip_editing/editing_split.json" 
    gcd_root_dir =  "/data/qj/editing/garmentcodedata_editing"
    dataset = RTDataset(
        [],
        split_file,
        gcd_root_dir,
